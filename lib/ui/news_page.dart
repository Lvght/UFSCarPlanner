...
class NewsPage extends StatelessWidget {
  ...
  RichText copo(String src){
    ...
    for(int i=0;i<x.length;i++){
      ...
      a=TextStyle(fontSize: 18, height: 1.5,
      fontWeight: tags.contains("em")||tags.contains("strong")?FontWeight.bold:FontWeight.normal,
      decoration: tags.contains("u")?TextDecoration.underline:TextDecoration.none,
      fontStyle: tags.contains("i")?FontStyle.italic:FontStyle.normal,
      color: tags.contains("a")?Colors.deepPurple:Colors.black,);

      TextSpan t = new TextSpan(text: text,style: a, recognizer: tags.contains("a")?( new TapGestureRecognizer()..onTap=
        () async {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        Container(
            child: teste,width: MediaQuery. of(context). size. width,)
          ],
        ),
      ),
    );
